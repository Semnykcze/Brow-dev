...
async function createTab(url = 'https://www.yahoo.com', titleText =  "", faviconUrl = "") {
        ...
        try {
            ...
            request.on('response', (response) => {
                ...
                response.on('end', () => {
                    ...
                    if (!faviconUrl) {
                        const favMatch = html.match(/<link[^>]+rel=["']?(?:shortcut )?icon["']?[^>]*>/i);
                        if (favMatch) {
                            ...
                            if (hrefMatch) {
                                ...
                                if (!favHref.startsWith('http')) {
                                    try {
                                        const u = new URL(url);
                                        favHref = u.origin + (favHref.startsWith('/') ? favHref : '/' + favHref);
                                    } catch {}
                                }
                                faviconUrl = favHref;
                            }
  ...
  /*   if (!titleText || !faviconUrl) {
    ...
    favicon.src = faviconUrl && faviconUrl.trim() !== '' ? faviconUrl : '../assets/brow.png';
    ...
    closeBtn.addEventListener('click', (e) => {
        ...
        const webviewToRemove = document.querySelector(`webview[data-tab-id="${tabIdToRemove}"]`);
    ...
    loadingBar.style.background = 'linear-gradient(90deg, #4285f4 0%, #60a5fa 100%)';
            ...
            if (activeWebview && activeWebview.src) {
                if (activeWebview.src.startsWith('https://')) {
                    ...
                    try {
                        ...
                        displayUrl = urlObj.host + urlObj.pathname + urlObj.search + urlObj.hash;
            ...
            urlInput.addEventListener('keydown', (event) => {
                if (event.key === 'Enter') {
                    if (activeWebview) {
                        ...
                        if (urlInput.dataset.realurl && val === urlInput.dataset.realurl.replace(/^https:\/\//, '')) {
...
function startFaviconsRefresh() {
    ...
    faviconsRefreshTimer = setInterval(() => {
        ...
        if (webview) {
            ...
            `, true).then(favUrl => {
                if (favUrl) {
                    updateTabData(tabId, tab.querySelector('.tab-title')?.textContent || '', favUrl);
                }
            });
            if (webview.getFavicon) {
                const faviconUrl = webview.getFavicon();
                if (faviconUrl) {
                    updateTabData(tabId, tab.querySelector('.tab-title')?.textContent || '', faviconUrl);
                }
            }
        }
